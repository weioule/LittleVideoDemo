package com.example.littlevideodemo.adapter;
...
import com.example.littlevideodemo.glide.GlideApp;
import com.example.littlevideodemo.utils.Utility;
import com.example.littlevideodemo.view.DetailActivity;
...
//        ((TextView) holder.getView(R.id.comments_num)).setText(data.getConsumption().getReplyCount() + "");
        ...
        if (data.getConsumption().isHas_favour())
            ((LikeView) holder.getView(R.id.lv_icon)).setDefaultIcon(mContext.getResources().getDrawable(R.drawable.give_like));
