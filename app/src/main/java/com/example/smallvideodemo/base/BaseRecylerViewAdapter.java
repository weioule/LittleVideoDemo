...
public abstract class BaseRecylerViewAdapter<E> extends RecyclerView.Adapter<BaseViewHolder> {
            ...
            default:
                if (mDataLists != null && mDataLists.size() > 0) {
                    ...
                    if (null != e) {
                        ...
                        bindData(holder, e, holder.getLayoutPosition());
                    }
                }
                break;
        }
    ...
    protected abstract void bindData(BaseViewHolder holder, E bean, int adapterPosition);
    ...
    public void remove(int position) {
        synchronized (BaseRecylerViewAdapter.class) {
            if (position >= 0 && position < mHeaderLayout.getChildCount()) {
                mHeaderLayout.removeViewAt(position);
            } else if (mDataLists != null && position >= 0 && mDataLists.size() > 0 && position < mDataLists.size()) {
...
//            mHeaderLayout.removeView(childAt);
    ...
    public void removeHeaderView(int index) {
        if (getHeaderLayoutCount() <= 0 || mHeaderLayout.getChildCount() <= 0 || index < 0 || index >= mHeaderLayout.getChildCount())
            return;
    ...
    public void removeHeaderView(View header) {
        if (getHeaderLayoutCount() <= 0 || mHeaderLayout.getChildCount() <= 0 || null == header)
            return;
...
//        if (null != childAt)
    ...
    public void removeFooterView(int index) {
        if (getFooterLayoutCount() == 0 || mFooterLayout.getChildCount() <= 0 || index <= 0 || index >= mFooterLayout.getChildCount())
            return;
    ...
    public void removeFooterView(View footer) {
        if (getFooterLayoutCount() == 0 || mFooterLayout.getChildCount() <= 0 || null == footer)
            return;
