 ...
 * on 2019/5/17.
 */
public class SmallVideoLikeView extends View implements Checkable {
    ...
    public SmallVideoLikeView(Context context) {
        this(context, null);
    }

    public SmallVideoLikeView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SmallVideoLikeView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        TypedArray array = context.getTheme().obtainStyledAttributes(attrs, R.styleable.SmallVideoLikeView, defStyleAttr, 0);
        mRadius = array.getDimension(R.styleable.SmallVideoLikeView_cirRadius, dp2px(10));
        mCycleTime = array.getInt(R.styleable.SmallVideoLikeView_cycleTime, DEFAULT_CYCLE_TIME);
        mUnSelectCycleTime = array.getInt(R.styleable.SmallVideoLikeView_unSelectCycleTime, DEFAULT_UN_SELECT_CYCLE_TIME);
        mDefaultColor = array.getColor(R.styleable.SmallVideoLikeView_defaultColor, DEFAULT_COLOR);
        mCheckedColor = array.getColor(R.styleable.SmallVideoLikeView_checkedColor, CHECKED_COLOR);
        mRingColor = array.getColor(R.styleable.SmallVideoLikeView_ringColors, DEFAULT_RING_COLOR);
        mLrGroupCRatio = array.getFloat(R.styleable.SmallVideoLikeView_lrGroupCRatio, HeartShapePathController.LR_GROUP_C_RATIO);
        mLrGroupBRatio = array.getFloat(R.styleable.SmallVideoLikeView_lrGroupBRatio, HeartShapePathController.LR_GROUP_B_RATIO);
        mBGroupACRatio = array.getFloat(R.styleable.SmallVideoLikeView_bGroupACRatio, HeartShapePathController.B_GROUP_AC_RATIO);
        mTGroupBRatio = array.getFloat(R.styleable.SmallVideoLikeView_tGroupBRatio, HeartShapePathController.T_GROUP_B_RATIO);
        mInnerShapeScale = array.getInteger(R.styleable.SmallVideoLikeView_innerShapeScale, RADIUS_INNER_SHAPE_SCALE);
        mDotSizeScale = array.getInteger(R.styleable.SmallVideoLikeView_dotSizeScale, DOT_SIZE_SCALE);
        mAllowRandomDotColor = array.getBoolean(R.styleable.SmallVideoLikeView_allowRandomDotColor, true);

        if (array.hasValue(R.styleable.SmallVideoLikeView_defaultLikeIconRes)) {
            mDefaultIcon = array.getDrawable(R.styleable.SmallVideoLikeView_defaultLikeIconRes);
        }
        if (array.hasValue(R.styleable.SmallVideoLikeView_checkedLikeIconRes)) {
            mCheckedIcon = array.getDrawable(R.styleable.SmallVideoLikeView_checkedLikeIconRes);
        }
    ...
    private void drawInnerShape(Canvas canvas, int radius, boolean isChecked) {
        if (isHasIcon()) {
            ...
            icon.setBounds(-radius - 5, -radius, radius + 5, radius);
            icon.draw(canvas);
        } else {
